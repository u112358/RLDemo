...
$5.3$~hours with threshold $0.97$. \todo{report update counts and throughput for both runs.}
...
run; the greedy success on uniformly random states implied by it is about $16\%$
(\todo{replace by the directly measured value}).
...
and threshold, so the comparison between them is qualitative. The greedy success on random
states of the main run is inferred from the per-distance table rather than measured directly.
...
\todo{repository URL}. All runs are reproducible from the command lines in the appendix.

\bibliographystyle{plainnat}
\begin{thebibliography}{99}
\providecommand{\natexlab}[1]{#1}
% \todo{verify every entry before submission}
...
\newblock In \emph{International Conference on Learning Representations}, 2019. (arXiv:1805.07470, 2018.)
